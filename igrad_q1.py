...
def iterate_all_word_length_rec(usable_list, selected_num_list, prefix, n, k, length_tup):
    ...
    # print prefix 
    if (k == 0) :
        for j in range(length_tup):
            word = selected_num_list[j]+ prefix
            short_collated_list.append(word)
...
num = int(input("Starting Digit: "))
word_length = int(input("Word Length: "))
row = 0
column = 0

if num == 1:
    print("Starting digit '1' does not have any alphabets associated with the number. Therefore, no output is produced.")
...
length_word = word_length - 1
length_tup = len(selected_tup)

collated_list = []

iterate_all_word_length(usable_alpha, selected_tup, length_word, length_tup)

collated_list = removeDuplicates(collated_list)
collated_list.sort()

print("Output: %s" % collated_list)
##to check total number of output applicable
## print(len(collated_list))